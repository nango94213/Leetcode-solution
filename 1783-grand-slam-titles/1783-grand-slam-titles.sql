...
cte2 as(select player_id, player_name, count(player_id) grand_slams_count from Players p join cte c on p.player_id = c.counting group by player_id order by grand_slams_count asc)
